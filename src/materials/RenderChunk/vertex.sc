...
void main() {
  ...
  vec4 color;
    ...
    vec4 fogColor;
    fogColor.a = clamp(((((camDis / FogAndDistanceControl.z) + RenderChunkFogAlpha.x) - (0.25 + FogAndDistanceControl.x)) / (0.25 + (FogAndDistanceControl.y - FogAndDistanceControl.x))), 0.0, 1.0);
